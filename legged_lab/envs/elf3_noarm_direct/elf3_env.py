...
from legged_lab.envs.elf3_noarm_direct.walk_cfg import Elf3WalkNoarmDirectFlatEnvCfg
...
class Elf3NoarmDirectEnv(VecEnv):
    def __init__(
        self,
        # cfg: (
        #     Elf3RunFlatEnvCfg
        ...
        # 手臂ik解
        import os
        floder = os.path.abspath(os.path.dirname(__file__))
        self.l_chain_ik = Chain.from_urdf_file(os.path.join(floder,"datasets/elf3_arm_l.urdf"), active_links_mask=[False] + [True]*7)
    ...
    def init_buffers(self):
        ...
        self.all_arms_ids,_ = self.robot.find_joints(
            name_keys=[
                "l_shoulder_y_joint",
                "l_shoulder_x_joint",
                "l_shoulder_z_joint",
    ...
    def random_sphere_pose(self,radius=0.6):
        """
        在半径为radius的球体内生成随机位置和随机四元数姿态

        参数:
            radius (float): 球体半径，默认为0.6米

        返回:
            tuple: (position, quaternion)
                position: 长度为3的numpy数组 [x, y, z]，单位为米
                quaternion: 长度为4的numpy数组 [w, x, y, z]，单位四元数
        """
        # 1. 生成球体内均匀随机位置（使用立方根采样法避免靠近中心聚集）
        # 先随机单位立方体内的点，再归一化到球体内
        while True:
            pos = np.random.uniform(-1, 1, 3)
            if np.linalg.norm(pos) <= 1:  # 确保在单位球内
                break
        # 乘以半径并立方根修正，使点在体积上均匀分布
        # 实际球体内均匀分布需要半径修正（因为体积元素与r^2成正比）
        r = radius * (np.random.random() ** (1 / 3))  # 半径按体积均匀分布
        position = pos / np.linalg.norm(pos) * r

        # 2. 生成随机归一化四元数（表示随机姿态）
        # 方法：在四维超球面上均匀采样
        u = np.random.rand(3)
        # 使用Ken Shoemake的方法：基于三维均匀分布生成均匀四元数
        w = np.sqrt(1.0 - u[0]) * np.sin(2 * np.pi * u[1])
        x = np.sqrt(1.0 - u[0]) * np.cos(2 * np.pi * u[1])
    ...
    def resample_arms(self):
        ...
        if self.arm_pos_resample_count < (3.0 / self.step_dt):
