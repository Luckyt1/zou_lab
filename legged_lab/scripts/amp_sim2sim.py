...
class MujocoRunner:
    ...
    def adjust_command_vel(self, idx: int, increment: float) -> None:
        ...
        if idx == 1:
            self.command_vel[idx] += increment
            self.command_vel[idx] = np.clip(self.command_vel[idx], -0.5, 0.5)  # vel clip
            print(f"Adjusted command velocity: x={self.command_vel[0]:.2f}, y={self.command_vel[1]:.2f}")
        elif idx == 2:
            self.command_vel[idx] += increment
            self.command_vel[idx] = np.clip(self.command_vel[idx], -0.8, 0.8)  # vel clip
            print(f"Adjusted command velocity: x={self.command_vel[0]:.2f}, y={self.command_vel[1]:.2f}")
        else:
            self.command_vel[idx] += increment
            self.command_vel[idx] = np.clip(self.command_vel[idx], -0.5, 1.0)  # vel clip
            print(f"Adjusted command velocity: yaw={self.command_vel[2]:.2f}")
